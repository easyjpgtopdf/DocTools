...
def free_preview_bg():
    ...
    Supports both multipart/form-data (preferred) and base64 JSON (fallback)
    """
    start_time = time.time()
    
    try:
        # PHASE 3: Try multipart/form-data first (remove.bg style)
        image_bytes = None
        max_size = 512
        image_type = None
        
        if 'image' in request.files:
            # Multipart upload (preferred method - matches remove.bg)
            ...
            logger.info("⚠️ Processing base64 JSON upload (fallback mode)")
            data = request.json
            ...
            try:
                ...
                logger.error(f"Base64 processing error: {str(base64_err)}")
                return jsonify({
                    'success': False,
                    'error': 'Invalid image data',
                    'message': f'Failed to process image data: {str(base64_err)}. Please try uploading a different image.'
                }), 400
        else:
            return jsonify({
                'success': False,
                'error': 'Invalid request format',
                'message': 'Request must be either multipart/form-data with image file, or JSON with imageData.'
            }), 400
        
        # Common processing path (for both multipart and base64)
        ...
        # image_type already set above (from multipart or base64)
