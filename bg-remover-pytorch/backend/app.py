...
from scipy.ndimage import binary_erosion, binary_dilation, distance_transform_edt, median_filter, uniform_filter
...
def preprocess_image(image, target_size=768):
    """Preprocess PIL image for U2Net Full - PROFESSIONAL: Higher resolution for better quality like remove.bg"""
    ...
    # Professional-grade resolution - Higher resolution = Better quality (like remove.bg, imagly)
    # Market leaders use 768-1024 for best quality
    max_dim = max(width, height)
    if max_dim > 3000:
        target_size = 1024  # Very large images - maximum quality
    elif max_dim > 1500:
        target_size = 768  # High quality - professional standard
    elif max_dim > 800:
        target_size = 768  # Standard - best balance for quality
    else:
        target_size = 512  # Small images - maintain aspect ratio
...
def postprocess_mask(mask_tensor, original_size, model_size):
    """Postprocess mask tensor to PIL Image - PROFESSIONAL: Enhanced edge preservation like remove.bg"""
    ...
    # Professional edge refinement - like remove.bg
    # Step 1: Light Gaussian smoothing for noise reduction (preserve details)
    mask_float = mask.astype(np.float32)
    mask_float = ndimage.gaussian_filter(mask_float, sigma=0.5)
    
    # Step 2: Edge-aware enhancement - sharpen edges while preserving smooth transitions
    # Use bilateral filter concept: preserve edges while smoothing
    from scipy.ndimage import uniform_filter
    mask_smooth = uniform_filter(mask_float, size=3)
    # Blend: keep sharp edges, smooth flat areas
    edge_mask = np.abs(mask_float - mask_smooth) > 10  # Detect edges
    mask_float = np.where(edge_mask, mask_float, mask_smooth * 0.7 + mask_float * 0.3)
    
    # Step 3: Final light smoothing for natural transitions
    mask_float = ndimage.gaussian_filter(mask_float, sigma=0.3)
    
    mask = np.clip(mask_float, 0, 255).astype(np.uint8)
    
    # Resize to original size with high-quality resampling (LANCZOS for best quality)
    mask_pil = Image.fromarray(mask, mode='L')
    mask_pil = mask_pil.resize(original_size, Image.LANCZOS)
    
    return mask_pil

def alpha_matting(image, mask, foreground_threshold=240, background_threshold=10, erode_size=15):
    """Apply alpha matting for professional edge quality - MARKET STANDARD: Like remove.bg/imagly"""
    try:
        ...
        # Professional thresholding - adaptive based on mask distribution
        # Use Otsu-like thresholding for better separation
        mask_mean = np.mean(mask_array)
        threshold = max(0.4, min(0.6, mask_mean))  # Adaptive threshold
        binary_mask = (mask_array > threshold).astype(np.uint8)
        
        # Create trimap for alpha matting (foreground, background, unknown)
        # Larger erode_size for better edge handling (professional standard: 15-20px)
        trimap = np.zeros_like(binary_mask, dtype=np.uint8)
        trimap[binary_mask == 1] = 255  # Definite foreground
        
        # Create unknown region around edges - PROFESSIONAL: Larger unknown region
        ...
        # Professional alpha calculation - Closed-form matting approach
        ...
        # PROFESSIONAL: In unknown regions, blend distance-based alpha with original mask probability
        # This preserves fine details (hair, fur) while ensuring smooth edges
        unknown_mask = (trimap == 128)
        # Weighted blend: 70% original mask (preserves details), 30% distance-based (smooth edges)
        alpha_unknown = mask_array[unknown_mask] * 0.7 + alpha[unknown_mask] * 0.3
        alpha[unknown_mask] = alpha_unknown
        
        # Professional edge refinement - Multi-stage processing
        # Stage 1: Edge-aware smoothing (preserve sharp edges, smooth flat areas)
        alpha_smooth = ndimage.gaussian_filter(alpha, sigma=1.0)
        edge_mask = (trimap == 128)  # Only smooth unknown regions
        alpha = np.where(edge_mask, alpha_smooth, alpha)
        
        # Stage 2: Median filter for noise reduction in edge regions only
        alpha_median = median_filter(alpha, size=3)
        alpha = np.where(edge_mask, (alpha * 0.6 + alpha_median * 0.4), alpha)
        
        # Stage 3: Final light smoothing for natural transitions
        alpha = ndimage.gaussian_filter(alpha, sigma=0.4)
        
        # Stage 4: Ensure crisp boundaries - sharpen edges slightly
        # Use unsharp mask technique for edge enhancement
        alpha_blur = ndimage.gaussian_filter(alpha, sigma=1.5)
        alpha_sharp = alpha + (alpha - alpha_blur) * 0.2  # 20% sharpening
        alpha = np.clip(alpha_sharp, 0, 1)
        
        # Final: Ensure proper range and clean background
        alpha = np.clip(alpha, 0, 1)
...
def remove_background_pytorch(image):
    ...
    # Preprocess with adaptive target size (higher resolution for better quality)
    ...
    with torch.no_grad():
        ...
        # PROFESSIONAL: Combine d0 and d1 for better edge quality (like remove.bg)
        if isinstance(raw_output, (tuple, list)):
            if not raw_output:
                raise ValueError("Model returned no outputs")
            
            # d0 is the final refined output
            d0 = raw_output[0]
            # d1 is the first refinement stage - often has better edge details
            if len(raw_output) > 1:
                d1 = raw_output[1]
                # Professional combination: 80% d0 (refined) + 20% d1 (edge details)
                # This preserves fine details while maintaining overall quality
                mask_tensor = 0.8 * d0 + 0.2 * d1
            else:
                mask_tensor = d0
        else:
    ...
    # Apply professional alpha matting for better edges (hair, fur, cloth)
    # Larger erode_size (15px) for professional-quality edge refinement
    alpha_array = alpha_matting(image, mask_pil, foreground_threshold=240, background_threshold=10, erode_size=15)
